...
int get_ipid_sequence(int numSamples, long *ipids, int islocalhost) {
  u32 ipid_diffs[32];
  int i, j, k, l;
...
void HostOsScan::makeTSeqFP(HostOsScanStats *hss) {
  ...
  if (good_tcp_ipid_num >= 3) {
    tcp_ipid_seqclass = get_ipid_sequence(good_tcp_ipid_num, (int_least64_t *) hss->ipid.tcp_ipids, islocalhost(hss->target->TargetSockAddr()));
  } else {
  ...
  if (good_tcp_closed_ipid_num >= 2) {
    tcp_closed_ipid_seqclass = get_ipid_sequence(good_tcp_closed_ipid_num, (int_least64_t *) hss->ipid.tcp_closed_ipids, islocalhost(hss->target->TargetSockAddr()));
  } else {
    tcp_closed_ipid_seqclass = IPID_SEQ_UNKNOWN;
  }

  if (good_icmp_ipid_num >= 2) {
    icmp_ipid_seqclass = get_ipid_sequence(good_icmp_ipid_num, (int_least64_t *) hss->ipid.icmp_ipids, islocalhost(hss->target->TargetSockAddr()));
  } else {
